...
def entry_buy_OBV(OBV, mv_OBV): 
	"""
	Determine the entry for buying 
	Input:
		OBV: a 2 X 1 array containing OnBalanceVolume for two consecutive days 
		mv_OBV: a 2 X 1 array containing moving averages for two consecutive days
	Ouput:
		Boolean: True if entry happens otherwise False
	"""
	if OBV[1] > mv_OBV[1] and OBV[0] <= mv_OBV[0]: 
		return True 
	else: 
		return False

def entry_sell_OBV(OBV, mv_OBV):
	"""
	Determine the entry for selling 
	Input:
		OBV: a 2 X 1 array containing OnBalanceVolume for two consecutive days 
...
def exit_trendreverse_buy_MA(mv, cp):
	"""
...
def exit_trendreverse_sell_MA(mv, cp):
	"""
	Determine exit status due to trend reversal. 
	Input:
		mv: a scalar denoting the moving average of the last N days
...
def exit_trendreverse_buy_OBV(mv_OBV, OBV):
	"""
	Determine exit status due to trend reversal. 
	Input:
		mv_OBV: a scalar denoting the moving average of the last N days
		OBV: a scalar denoting the OnBalanceVolume of the same day. 
	Output:
		Boolean: True if exit happens and False otherwise.
	"""
	if OBV < mv_OBV:
...
def exit_trendreverse_sell_OBV(mv_OBV, OBV):
	"""
	Determine exit status due to trend reversal. 
	Input:
		mv_OBV: a scalar denoting the moving average of the last N days
		cp_OBV: a scalar denoting the OnBalanceVolume of the same day. 
	Output:
		Boolean: True if exit happens and False otherwise.
	"""
	if OBV > mv_OBV:
...
def trade_stock_MA_1(mv, op, cp, varloss, vargain, N=20, alpha=0.05, replication=1000):
	"""
...
def trade_stock_MA_2(mv, op, cp, vargain, N=20, alpha=0.05, replication=1000):
	"""
	Trade stock based on opening, closing and moving average price by executing various entry and exit rules. 
	Input:
		mv: a T numpy array of moving average. 
...
def trade_stock_MA_3(mv, op, cp, varloss, N=20, alpha=0.05, replication=1000):
	"""
	Trade stock based on opening, closing and moving average price by executing various entry and exit rules. 
	Input:
		mv: a T numpy array of moving average. 
	...
	for t in range(N+1,T):
            ...
            if status is "none": 
                ...
                if status is "long" and (exit_trendreverse_buy(mv[t],cp[t]) or exit_varloss(op[entrytime],cp[t],varloss, status) or (t-entrytime>N): #exit after 20 days (same number of days as N in ma)  
                    profit_list.append(np.log(cp[t]/op[entrytime]))
                    if exit_trendreverse_buy(mv[t],cp[t]):
                        status = "short"
                    else:
...
def trade_stock_MA_4(mv, op, cp, varloss, N=20, alpha=0.05, replication=1000):
	"""
	Trade stock based on opening, closing and moving average price by executing various entry and exit rules. 
	Input:
		mv: a T numpy array of moving average. 
...
def trade_stock_MA_1(mv, mv_OBV, op, cp, OBV, varloss, vargain, N=20, alpha=0.05, replication=1000):
	"""
	Trade stock based on opening, closing and moving average price by executing various entry and exit rules. 
	Input:
		mv: a T numpy array of moving average. 
...
m_OBV = dob.doataobj("OBV.csv")
mv_cp = moving_average(m_cp.data, N=N)
mv_OBV = moving_average(m_OBV, N=N)
...
profit_list,profit,status_list,entry_list,exit_list,entry_price_list,exit_price_list= trade_stock(mv_cp[choice], m_op.data[choice], m_cp.data[choice], varloss, vargain, N=N, alpha=alpha)
